...
I wrote this as a simple text based RPG in Python. It uses an OpenAI model to generate the world, scenes, and decide when to enter combat, and it saves progress under `logs/`. The intiial idea was to make a playable book adventure where you can start in a fantasy world with an LLM serving as a sort of "dungeon master" and you can do literally anything you want afterwards. The way I thought to implement this was building a system where the LLM doesn't own the entire game state directly, but still manages narrative freedom.
...
2. Install the Python dependencies `Pygame` and `Pydantic` and `python-dotenv`, as well as `openai` (or the dependencies for your model of choice):
```bash 
...
`game_ui.py` is where the LLM request gets built. It sends the current engine state, the recent hot context, and any cold logs I pull back in from disk. It also handles the simple local commands like inventory, health, time, location, equip, and help before the request ever reaches the model.
...
Combat is simple on purpose: attack, use item, or flee. The model can trigger a fight by returning a combat encounter, but the damage rolls and HP changes happen locally, not in the narrative text.

I also wrote `eval.py` as a regression harness for the LLM contract. If I change the system prompt, I can run those checks against a set of sample inputs and make sure the model still returns valid JSON, respects the schema, and follows the basic game rules.

The master system prompt in `system_prompt.md` gives strict restrictions on what the LLM can and cannot do. Given more powerful, creative models, this could be altered to go in many new directions and allow more freedom for the LLM to generate a more interesting, fleshed out game. 
